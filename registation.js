/*
@getValueOfApi
** call this function for show data in table
*/
getValueOfAPi();
...
function editeRow() {
    ...
    if(valAll(name1,number,email,city,state,country,pin)){
        ...
        fetch(url,param).then(response =>{
            ...
            row.childNodes[8].innerHTML = '<button class="btn edit" data-bs-toggle="modal" data-bs-target="#editModal"'
...
/*
@getValueOfApi
** this function is use to call api and get data from api
 */
function getValueOfAPi() {
    var url = "http://192.168.0.107:8080/students";
    var param = {
        method:"GET",
        headers:{
    ...
    fetch(url,param).then(response =>{
        ...
        if (data.length > 0) {
            ...
            data.forEach((itemData) => {
                ...
                temp += "<td>" + '<button class="btn edit" data-bs-toggle="modal" data-bs-target="#editModal"' +
                    'onclick="rowDetalis(this); showEditeValue();"><i class="fas fa-edit"></i></button>' +
                    '<button class="btn delete" data-bs-toggle="modal" data-bs-target="#exampleModal"' +
                    'onclick="rowDetalis(this)"><i class="fas fa-trash-alt" style="color: red "></i></button>' + 
                    "</td></tr>";
